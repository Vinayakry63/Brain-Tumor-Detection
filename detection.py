...
dataset_path = r"D:\BrainTumor\brain-tumor-mri-dataset"  # <-- FIX: added r

# Define the training and testing directories
train_dir = os.path.join(dataset_path, "Training")       # <-- FIX: clean join
test_dir = os.path.join(dataset_path, "Testing")         # <-- FIX: clean join
...
confusion_matrix_np = confusion_matrix.numpy()  # Convert to numpy array
precision = np.diag(confusion_matrix_np) / np.sum(confusion_matrix_np, axis=0)
recall = np.diag(confusion_matrix_np) / np.sum(confusion_matrix_np, axis=1)
